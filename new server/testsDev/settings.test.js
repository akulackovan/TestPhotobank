...
describe("POST /settings/ check find user", () => {
  ...
  it("Should give error with id string is not objectId", async () => {
    const userId = "41224d";
    const res = await request(app)
      .post("/settings")
      .send({ userId: userId, username: "" , password: "" ,
      ...
      base64: ""});
    expect(res.statusCode).toEqual(400);
    expect(res.body.message).toBe("Ошибка при изменении настроек.");
  });
});
...
describe("POST /settings/ ", () => {
    ...
    it("Should find empty password field with correct username and error in city", async () => {
      ...
      const res = await request(app)
        .post("/settings")
        .send({ userId: userId, username: "ЕнвроывНЕроVGHsh" , password: "" ,
        newpass: "тест" ,
        checkpass: "тест" ,
        text: "" ,
        ...
        base64: photo});
      expect(res.statusCode).toEqual(400);
      expect(res.body.message).toBe("Не все поля для изменения пароля введены");
    });
    ...
    it("Should give an error stating that the login is busy with error in password", async () => {
        ...
        const res = await request(app)
          ...
          base64: ""});
        expect(res.statusCode).toEqual(400);
        expect(res.body.message).toBe("Логин занят. Выберите другой");
      });
      ...
      it("Should give an error stating that the login is busy with error in text", async () => {
        ...
        const res = await request(app)
          .post("/settings")
          .send({ userId: userId, username: "тест" , password: "тест" ,
          newpass: "тест" ,
          checkpass: "тест" ,
          text: photo,
          city: city,
      ...
      it("Should give an error in text with error in city and correct other field", async () => {
        ...
        expect(res.body.message).toBe("Описание пользователя содержит больше 512 символов.");
      });
      ...
      it("Should give an error in password with correct other field", async () => {
        ...
        const res = await request(app)
          .post("/settings")
          .send({ userId: userId, username: "" , password: "тест12" ,
          newpass: "" ,
          checkpass: "тест" ,
          text: "Это тест",
          city: city,
      ...
      it("Should give an error in uncorrect password for user with correct other field", async () => {
        ...
        const res = await request(app)
          .post("/settings")
          .send({ userId: userId, username: "" , password: "тест1122" ,
          newpass: "тест" ,
          checkpass: "тест" ,
          text: "Это тест",
          city: city,
      ...
      it("Should give an error in password with uncorrect text and correct other field with change login", async () => {
        ...
        const res = await request(app)
          .post("/settings")
          .send({ userId: userId, username: "тест3333" , password: "тест" ,
          newpass: "" ,
          checkpass: "" ,
          text: photo,
          city: city,
      ...
      it("Should give an error in uncorrect password with correct other field", async () => {
        ...
        const res = await request(app)
          .post("/settings")
          .send({ userId: userId, username: "тест3333" , password: "тест122" ,
          newpass: "тест" ,
          checkpass: "тест1222" ,
          text: "Это тест",
          city: "",
      ...
      it("Should give an error in city with correct other field", async () => {
        ...
        const res = await request(app)
          .post("/settings")
          .send({ userId: userId, username: "тест3333" , password: "" ,
          newpass: "" ,
          checkpass: "" ,
          text: "",
          city: userId,
          base64: ""});
        expect(res.statusCode).toEqual(400);
        expect(res.body.message).toBe("Такого города нет");
      });
      ...
      it("Should give an error different password with uncorrect text", async () => {
        ...
        const res = await request(app)
          ...
          base64: photo});
        expect(res.statusCode).toEqual(400);
        expect(res.body.message).toBe("Пароли не совпадают.");
      });
      ...
      it("Should give an error empty password with correct and empty other fields", async () => {
        ...
        const res = await request(app)
          .post("/settings")
          .send({ userId: userId, username: "тест3333" , password: "тест111" ,
          newpass: "" ,
          checkpass: "" ,
          text: "Тест",
          city: "",
      ...
      it("Should give an error in busy login with uncorrect password and uncorrect other fields", async () => {
        ...
        const res = await request(app)
          .post("/settings")
          .send({ userId: userId, username: "тест" , password: "тестпрпор" ,
          newpass: "" ,
          checkpass: "тест123" ,
          text: "Тест",
          city: userId,
      ...
      it("Should give an error in busy login with empty password and uncorrect city", async () => {
        ...
        const res = await request(app)
          .post("/settings")
          .send({ userId: userId, username: "тест" , password: "тест123" ,
          newpass: "тест" ,
          checkpass: "тест" ,
          text: "Это тест",
          city: userId,
